        ...
        try {
            ...
            for (let istage = 1; istage <= stageNumber; istage++){
                ...
                stageresults.forEach(result => {
                    ...
                    if (groupresults[result.bib] && result.status !== "unknown" && result.status !== "abs"){
                        groupresults[result.bib].time       += result.time - groupresults[result.bib].bonification;
                    ...
                    } else if (istage == 1 && !groupresults[result.bib]) {
                        groupresults[result.bib] = {...result, stage: istage, cumulated: result.rank};
                        groupresults[result.bib].time -= groupresults[result.bib].bonification;
                    }

                });
            }

            const allresults = Object.values(groupresults).sort((a, b) => {
            ...
            res.status(200).json({
                stage: stageNumber,
                results: allresults.map(r => ({...r, time: this._time.formatHMS(r.time), delay: this._time.formatHMS(r.time - allresults[0].time)}))
            });
